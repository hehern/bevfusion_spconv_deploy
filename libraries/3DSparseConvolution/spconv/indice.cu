...
int create_submconv_indice_pair_cuda(
  ...
  cuda_linear_launch(prepareSubMGridKernel, _stream, numActIn, indicesIn, gridsOut, outSpatialShape, spatialVolume);//建立输出张量坐标(通过index表示)到输出序号之间的一张哈希表
  cuda_linear_launch(getSubMIndicePairsKernel3, _stream, numActIn, indicesIn, gridsOut, indicePairs, indiceNum, outSpatialShape, spatialVolume);
